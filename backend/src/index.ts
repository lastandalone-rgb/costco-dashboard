...
const app = express();
app.use(cors());
app.use(express.json({ limit: '10mb' })); // Facebook GraphQL 回應可能很大
...
// 廣播函式
const broadcastNewPost = (postData: any) => {
...
const extractInfo = (content: string) => {
  ...
  if (!content) return { price, store_location };
...
app.post('/api/posts', async (req, res) => {
  try {
    ...
    // Upsert Post (新增或更新)
    await pool.query(
      `INSERT INTO posts (id, author_name, author_avatar, content, created_at, price, store_location, images, raw_data, likes_count, comments_count, shares_count, last_updated)
    ...
    if (Array.isArray(comments) && comments.length > 0) {
      for (const comment of comments) {
        await pool.query(
          `INSERT INTO comments (id, post_id, author_name, author_avatar, content, created_at, raw_data)
           ...
           ON CONFLICT (id) DO UPDATE SET 
             ...
             raw_data = EXCLUDED.raw_data`,
          [comment.id, id, comment.author_name, comment.author_avatar || '', comment.content, comment.created_at ? new Date(comment.created_at) : new Date(), comment.raw_data]
        );
      }
    }
    ...
    const broadcastQuery = `
      ...
      GROUP BY p.id
    `;
    const dbResult = await pool.query(broadcastQuery, [id]);
